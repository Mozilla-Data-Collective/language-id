...
|---|---|---|
| `finetune` | `uv sync --extra finetune` | `torch`, `transformers`, `datasets`, `accelerate` — needed to fine-tune a Hugging Face model as a single-language detector (`language-id train --method llm`). |
...
This lists every model name the CLI can evaluate. Then run a small first evaluation:

```bash
uv run language-id eval --model langdetect --dataset commonlid --n 50
```
